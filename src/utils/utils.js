 ...
 * @param allowCero
 * @param allowNegative
 * @returns {boolean} If the string is a valid number
 */
...
const sortByDate = (array = [], field = 'created') => {
  return array
    ...
    .filter(
      (current, index, array) =>
        index === 0 ||
        new Date(current[field]).getTime() !== new Date(array[index - 1][field]).getTime(),
    );
};
...
function debounce(func, wait, c) {
  ...
  return function() {
    ...
    var f = this,
      g = arguments;
    return clearTimeout(d), (d = setTimeout(h, wait)), c && !d && (e = func.apply(f, g)), e;
  };
}
